...
func int Spell_ProcessMana(var int manaInvested)
{
	...
	{
		return Spell_Logic_FirerainInv(manaInvested);
	};
	...
	if(Npc_GetActiveSpell(self) == SPL_HEALNEARBY)
	{
		return Spell_Logic_HealNearby(manaInvested);
	};
	if(Npc_GetActiveSpell(self) == SPL_HEALNEARBY2)
	{
		return Spell_Logic_HealNearby2(manaInvested);
	};
	return 0;
};
