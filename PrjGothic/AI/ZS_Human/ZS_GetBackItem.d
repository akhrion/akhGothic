...
func void B_GetBackItem_Given()
{
	...
	if(self.aivar[AIV_WANTEDITEM] == Hlp_GetInstanceID(item))
	{
		PrintDebugNpc(PD_ZS_FRAME,"ZS_GetBackItem_Given // want this item cause itґs mine");
		Npc_ClearAIQueue(self);
	...
	else
	{
		if(C_WantItem())
		{
			PrintDebugNpc(PD_ZS_FRAME,"ZS_GetBackItem_Given // want this cause itґs worthy");
			Npc_ClearAIQueue(self);
			Npc_GiveItem(other,item,self);
		...
		{
			PrintDebugNpc(PD_ZS_FRAME,"ZS_GetBackItem_Given donґt want this crap");
			B_Say(self,other,"$YOUWANNAFOOLME");
			if(C_AmIStronger(self,other))
			{
				PrintDebugNpc(PD_ZS_FRAME,"ZS_GetBackItem_Given donґt want this crap // stronger");
				Npc_ClearAIQueue(self);
				Npc_SetTarget(self,other);
				AI_StartState(self,ZS_Attack,0,"");
			};
