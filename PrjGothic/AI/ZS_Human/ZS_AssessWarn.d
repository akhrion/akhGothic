...
func void ZS_AssessWarn()
{
	...
	if(
		(victim.aivar[AIV_ITEMSCHWEIN] == TRUE) &&
	...
	)
	{
		...
		(
			...
			Npc_WasInState(victim,ZS_CallGuardsOnEnterRoom)
		) &&
	...
	)
	{
		...
		PrintDebugNpc(PD_ZS_Check,"...Warnung von Schьtzling, dessen Raum betreten/verlassen wurde!");
		...
		(
			C_NpcIsGuard(self) || C_NpcIsGuardArcher(self) || C_NpcIsBoss(self)
		) && (
			Npc_WasInState(victim,ZS_GuardPassage) || Npc_IsInState(victim,ZS_GuardPassage)
		) && (Npc_GetAttitude(self,victim) == ATT_FRIENDLY)
	)
	{
		Print("PROCLAIN AND PUNISH");
...
func void ZS_AssessWarn_End()
{
	...
	if(Npc_CanSeeNpcFreeLOS(self,other))
	{
		...
		{
			PrintDebugNpc(PD_ZS_Check,"ZS_AssessWarn End // Free Los //Not HOSTILE Kumpel getцtet");
			Npc_GetTarget(self);
			AI_StartState(self,ZS_AssessEnemy,0,"");
		}
		else
	...
	{
		PrintDebugNpc(PD_ZS_Check,"ZS_AssessWarn  End// CanґT See");
		return;
	};
};
