...
{
	//Где-то здесь должна быть регулярная проверка на активное оружие у ГГ
	//Нередки случаи, когда НПС агрится на ГГ, когда тот быстро спрятал оружие и
	// что-бы снять агро требуется его снова передоставать и прятать
	...
	if(
		(
			...
			(
				C_NpcIsWorker(self) &&
				C_AmIWeaker(self,other)
			)
		) && Npc_CanSeeNpcFreeLOS(self,other)
	)
	{
		PrintDebugNpc(PD_ZS_Check,"...NSC wird zurьckweichen!");
...
//Print("zzz");
	...
	{
		PrintDebugNpc(PD_ZS_Check,"...SC ist auЯerhalb Fernkampfdistanz!");
		return LOOP_END;
	};
...
{
Print("can't stop now -> BUG");
	PrintDebugNpc(PD_ZS_LOOP,"ZS_AssessFighterWait_Loop");
	if(Npc_GetDistToNpc(self,other) > HAI_DIST_ABORT_MELEE)
	{
		PrintDebugNpc(PD_ZS_Check,"...SC ist auЯerhalb Nahkampfreichweite!");
