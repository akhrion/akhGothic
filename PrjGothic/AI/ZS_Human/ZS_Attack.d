...
func int ZS_Attack_Loop()
{
	...
	if(Npc_GetStateTime(self) > 2)
	{
		...
		if(
			Npc_IsPlayer(other) &&
			(
				C_BodyStateContains(other,BS_RUN) ||
...
func void ZS_Attack_End()
{
	...
	{
		PrintDebugNpc(PD_ZS_Check,"...Ziel bewuЯtlos/magisch schlafend und NSC kein purer Fernkдmpfer!");
	...
	if(!C_NpcIsBoss(self) && C_NpcIsHuman(other) && (Npc_IsInState(other,ZS_Unconscious) || Npc_IsDead(other) || Npc_IsInState(other,ZS_MagicSleep)))
	{
		...
		{
			PrintDebugNpc(PD_ZS_Check,"...Plьnderbedingungen erьllt!");
...
func void B_CombatFightTogether()
{
	...
	)
	{
		...
		{
			Print(ConcatStrings("We fight with it together, against: ", IntToString(hero.aivar[AIV_LASTTARGET])));
		}
		else
		{
			Print(ConcatStrings("What are you doint? Why you attack: ", IntToString(hero.aivar[AIV_LASTTARGET])));
		};
	};
};