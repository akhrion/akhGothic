...
func int ZS_WatchFight_Loop()
{
	...
	{
		PrintDebugNpc(PD_ZS_LOOP,"...Workaraound fьr nicht ankommende PERC_ASSESSMURDER!");
		return LOOP_END;
	};
	if(Npc_IsInState(other,ZS_Unconscious) || Npc_IsInState(victim,ZS_Unconscious))
	{
		PrintDebugNpc(PD_ZS_LOOP,"...Workaround fьr nicht ankommende PERC_ASSESSDEFEAT!");
		return LOOP_END;
	};
	if((Npc_IsInState(other,ZS_Attack) || Npc_IsInState(victim,ZS_Attack)) && (other != Kirgo) && (other != Kharim) && (other != GorHanis))
	{
		PrintDebugNpc(PD_ZS_Check,"...es wird noch gekдmpft!");
	...
	{
		PrintDebugNpc(PD_ZS_Check,"...es wird nicht mehr gekдmpft!");
		return LOOP_END;
	};
	AI_TurnToNPC(self,other);
	return LOOP_CONTINUE;
