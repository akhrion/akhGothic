...
func void B_AssessEnterRoom()
{
	...
	{
		PrintDebugNpc(PD_ZS_Check,"...Nsc sieht/hцrt Eindringling!");
		if(C_NpcIsGuard(self))
		{
			...
			{
				PrintDebugNpc(PD_ZS_Check,"...betretener Portalraum gehцrt Schьtzling-Gilde!");
			...
			{
				PrintDebugNpc(PD_ZS_Check,"...verlassener Portalraum gehцrt Schьtzling-Gilde!");
				if(!C_BodyStateContains(self,BS_MOBINTERACT_INTERRUPT))
				{
					B_FullStop(self);
				};
		...
		{
			PrintDebugNpc(PD_ZS_Check,"...ich bin Fernkampfwache und ignoriere Rдume betreten grundsдtzlich!");
		...
		else
		{
			...
			{
				PrintDebugNpc(PD_ZS_Check,"...SC nah & betretener Portalraum gehцrt Gilde des NSCs");
				if(!C_BodyStateContains(self,BS_MOBINTERACT_INTERRUPT))
				{
					B_FullStop(self);
				};
			...
			{
				PrintDebugNpc(PD_ZS_Check,"...SC nah & SC verlдЯt eigenen Portalraum");
				if(!C_BodyStateContains(self,BS_MOBINTERACT_INTERRUPT))
				{
					B_FullStop(self);
				};
	...
	{
		PrintDebugNpc(PD_ZS_Check,"...NSC kann den Eindringling weder sehen noch hцren!");
	};
};
