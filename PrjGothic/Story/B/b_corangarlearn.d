...
func void Info_CorAngar_FindHerb_Info()
{
	...
	B_LogEntry(CH3_FindHerbs,"Юберион без сознания. Кор Ангар попросил меня найти для него пять кустов самых сильных лечебных трав. Но поможет ли это..?");
};
...
func void Info_CorAngar_FindHerb_Success_Info___()
{
	...
	AI_Output(other,self,"Info_CorAngar_FindHerb_Success_NULL_05");	//Что? Этого не может быть! Юберион наверняка, что-то не так понял! А как-же магия Спящего? Магия, что он дает совершенно не агрессивна, она уникальна - мы обязаны изучить ее сильнее!
	AI_Output(self,other,"Info_CorAngar_FindHerb_Success_NULL_06");	//Может и так.
